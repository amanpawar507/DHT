...
//		routing.joinCheck();
	...
	public void listenOn(String key, EventListener listener) throws DHTBase.Failed {
		 ...
		 * 
		 * NOTE: You will actually have two types of events, and therefore two listeners on 
		 * the client side:
		 * 1. A listener for new binding events
		 * 2. A listener for moved binding events
		 * The latter is for the case where some of the bindings at a node move to another node 
		 * as a result of the latter node joining the network.  We need to notify listeners that
		 * the binding has moved.  They react to this by stopping and restarting the listening.
